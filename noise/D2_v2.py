...
def sites_2d(n, s):
    ...
    """
    myslice = [i*(1<<(n-s)) for i in range(2**s)]
...
def nn_xe(n, s):
    ...
    """
    sites = sites_2d(n,s)
    return [[sites[2*x,y], sites[(2*x+1)%(2**s),y]] for x in range(2**(s-1)) for y in range(2**s)]
...
def nn_xo(n, s):
    ...
    """
    sites = sites_2d(n,s)
    return [[sites[(2*x+1)%(2**s),y], sites[(2*x+2)%(2**s),y]] for x in range(2**(s-1)) for y in range(2**s)]
...
def nn_ye(n, s):
    ...
    """
    sites = sites_2d(n,s)
    return [[sites[x,2*y], sites[x,(2*y+1)%(2**s)]] for x in range(2**s) for y in range(2**(s-1))]
...
def nn_yo(n, s):
    ...
    """
    sites = sites_2d(n,s)
    return [[sites[x,(2*y+1)%(2**s)], sites[x,(2*y+2)%(2**s)]] for x in range(2**s) for y in range(2**(s-1))]
