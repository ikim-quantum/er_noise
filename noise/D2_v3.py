...
def sites_2d(n, s):
    ...
    """
    myslice = [i*(3**(n-s)) for i in range(3**s)]
    return {(x,y): (myslice[x], myslice[y]) for x in range(3**s) for y in range(3**s)}
...
def nn_xe(n, s):
    ...
    """
    sites = sites_2d(n,s)
    return [[sites[2*x,y], sites[(2*x+1)%(3**s),y]] for x in range(3**(s)//2) for y in range(3**s)]
...
def nn_xo(n, s):
    ...
    """
    sites = sites_2d(n,s)
    return [[sites[(2*x+1)%(3**s),y], sites[(2*x+2)%(3**s),y]] for x in range(3**(s)//2) for y in range(3**s)]
...
def nn_ye(n, s):
    ...
    """
    sites = sites_2d(n,s)
    return [[sites[x,2*y], sites[x,(2*y+1)%(3**s)]] for x in range(3**s) for y in range(3**(s)//2)]
...
def nn_yo(n, s):
    ...
    """
    sites = sites_2d(n,s)
    return [[sites[x,(2*y+1)%(3**s)], sites[x,(2*y+2)%(3**s)]] for x in range(3**s) for y in range(3**(s)//2)]
...
def convert_2d_to_1d(coordinate, n):
    ...
    Returns:
        int: 1D coordinate
    """
    return coordinate[0] + 3**n * coordinate[1]
